...
func (s *OptimizedCategoryService) CreateCategory(ctx context.Context, category *models.Category) error {
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if category.ParentID != nil {
			if err := s.validateParentCategoryTx(tx, *category.ParentID, category.CreatedBy, category.ID); err != nil {
				return err
			}
		}
...
func (s *OptimizedCategoryService) validateParentCategoryTx(tx *gorm.DB, parentID, userID, excludeID uint) error {
	// 一次性获取用户的所有分类，构建层级关系
	var allCategories []models.Category
	if err := tx.Where("created_by = ? AND deleted_at IS NULL", userID).
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if parentID, exists := updates["parent_id"]; exists {
			if parentID != nil {
				...
				if err := s.validateParentCategoryTx(tx, parentIDUint, userID, id); err != nil {
					return err
				}
			}
		}
		...
		if name, exists := updates["name"]; exists {
			...
			if err := tx.Where("name = ? AND created_by = ? AND id != ? AND deleted_at IS NULL", 
