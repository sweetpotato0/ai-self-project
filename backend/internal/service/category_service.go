...
func NewCategoryService(db *gorm.DB, logger logger.LoggerInterface) *CategoryService {
...
func (s *CategoryService) GetCategoryByID(id uint, userID uint) (*models.Category, error) {
	...
	if err := s.db.Where("id = ? AND created_by = ?", id, userID).
		Preload("Parent").
		Preload("Children").
...
func (s *CategoryService) GetCategoryTree(userID uint) ([]models.Category, error) {
	...
	if err := s.db.Where("created_by = ? AND parent_id IS NULL", userID).
		Preload("Children").
...
func (s *CategoryService) UpdateCategory(id uint, userID uint, updates map[string]interface{}) error {
	...
	// 检查分类是否存在且属于当前用户
	var category models.Category
	if err := s.db.Where("id = ? AND created_by = ?", id, userID).First(&category).Error; err != nil {
...
func (s *CategoryService) DeleteCategory(id uint, userID uint) error {
	...
	// 检查是否有关联的任务
	var todoCount int64
	if err := s.db.Model(&models.Todo{}).Where("category_id = ?", id).Count(&todoCount).Error; err != nil {
		return err
	}
	if todoCount > 0 {
...
func (s *CategoryService) loadChildrenRecursively(category *models.Category, userID uint) error {
	...
	if err := s.db.Where("parent_id = ? AND created_by = ?", category.ID, userID).
		Preload("Children").
		Order("sort ASC, id ASC").
...
func (s *CategoryService) checkCircularReference(parentID uint, userID uint) error {
	...
	// 检查父分类是否存在
	var parent models.Category
	if err := s.db.Where("id = ? AND created_by = ?", parentID, userID).First(&parent).Error; err != nil {
		return fmt.Errorf("父分类不存在: %w", err)
	}
	...
	for currentID != 0 {
		...
		if err := s.db.Where("id = ? AND created_by = ?", currentID, userID).First(&current).Error; err != nil {
			break
		}
...
func (s *CategoryService) GetCategoryStats(userID uint) (map[string]interface{}, error) {
	...
	if err := s.db.Model(&models.Category{}).Where("created_by = ?", userID).Count(&totalCount).Error; err != nil {
		return nil, err
	}
	...
	// 顶级分类数
	var rootCount int64
	if err := s.db.Model(&models.Category{}).Where("created_by = ? AND parent_id IS NULL", userID).Count(&rootCount).Error; err != nil {
		return nil, err
	}
	...
	// 子分类数
	var childCount int64
	if err := s.db.Model(&models.Category{}).Where("created_by = ? AND parent_id IS NOT NULL", userID).Count(&childCount).Error; err != nil {
		return nil, err
	}
