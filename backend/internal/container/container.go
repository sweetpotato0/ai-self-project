...
func (c *Container) initializeAllServices() {
	...
	notificationHandler := handler.NewNotificationHandler(notificationService, globalLogger, c.db)
	...
	websocketHandler := handler.NewWebSocketHandler(globalLogger, c.db)
